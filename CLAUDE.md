...
## Открытый вопрос: README не рендерится на странице профиля (2026-08-11)

На `github.com/Archangel-gabri/Archangel-gabri` README отображается нормально, а на
`github.com/Archangel-gabri` его блока нет вовсе. Проверено и исключено:

- имя репо == логин (побуквенно), репо публичный, `README.md` в корне `main` — всё верно;
- не кэш: `cache-control: max-age=0, private, must-revalidate`, cache-busting не помог;
- не содержимое: подмена на README из двух строк тоже не отобразилась;
- не разовый сбой рендера: три клиента (curl, headless chromium, Brave через CDP) — одинаково;
- переключение видимости private→public кэш не сбросило.

Остаётся гипотеза, которую нельзя проверить анонимно: GitHub скрывает profile README у аккаунтов,
помеченных как ограниченные/flagged. Владельцу под своей сессией README, скорее всего, виден.
Следующий шаг — посмотреть залогиненным; если и там пусто, писать в GitHub Support.
